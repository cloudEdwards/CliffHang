namespace Fusion.Addons.KCC
{
	public sealed class SmoothFloat : SmoothValue<float>
	{
		...
		{
			SmoothItem<float>[] items = Items;
			SmoothItem<float>   item;

			if (positive == true) { for (int i = 0, count = items.Length; i < count; ++i) { item = items[i]; if (item.Value > 0.0f) { item.Value = 0.0f; } } }
			if (negative == true) { for (int i = 0, count = items.Length; i < count; ++i) { item = items[i]; if (item.Value < 0.0f) { item.Value = 0.0f; } } }
		}
