...
{
	/// <summary>
	/// Stage executed after each move step (physics query, depenetration from colliders, updating collision hits), before firing OnEnter/OnExit callbacks.
	/// Use for position corrections, step detection, ground snapping, vector projections, ...
	/// This stage is executed only if pending delta time > KCCSettings.ExtrapolationDeltaTimeThreshold (0.05ms), otherwise extrapolation is used.
	/// </summary>
	...
	/// Refreshing KCCData.Hits after the stage can be explicitly requested if needed (typically after modifying KCCData.TargetPosition).
	/// </summary>
	public sealed class AfterMoveStep : IAfterMoveStep, IBeforeStage, IAfterStage
	{
		// PUBLIC MEMBERS

		public readonly KCCOverlapInfo OverlapInfo = new KCCOverlapInfo(KCC.CACHE_SIZE);
		...
		/// <summary>
		/// Request KCCData.Hits update after the stage. Metadata (collision type, penetration, ...) for new hits will be missing as they require full depenetration pass.
		/// </summary>
		...
		public void RequestUpdateHits(bool forceNewOverlapQuery)
		{
			...
			if (forceNewOverlapQuery == true)
			{
				_updateHitsOverlapQuery = EKCCHitsOverlapQuery.New;
			}
		}
		...
		{
			if (_updateHitsRequested == true)
			{
				kcc.UpdateHits(OverlapInfo, _updateHitsOverlapQuery);
			}
		}
	}
}
